...
from models.utils import MultiQueryAttention, SimpleFeedForward,CrossAttention
...
def img_to_text(x: Tensor, seqlen: int, dim: int, norm: bool = True):
    ...
    img = nn.Linear(h * w, dim).to("cuda")(img)
    img = rearrange(img, "b c d -> b d c")
    img = nn.Linear(c, seqlen).to("cuda")(img)
    img = rearrange(img, "b d c -> b c d")

    if norm:
        img = nn.LayerNorm(dim).to("cuda")(img)
